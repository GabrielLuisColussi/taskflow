...
import { deriveNameFromEmail, saveUserSession } from "../../../lib/utils/auth-user";
...
export default function LoginPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!response.success) {
        setMessage(response.message || "Erro no login");
        return;
      }
      ...
      saveUserSession({
        ...
        email: form.email,
        name: response.data?.user?.name || deriveNameFromEmail(form.email),
      });
        ...
        <section className="hidden border-r border-zinc-800/80 bg-zinc-900/50 p-10 lg:flex lg:flex-col lg:justify-between">
          <div>
            ...
            <h1 className="mt-6 max-w-lg text-5xl font-semibold tracking-tight">
              Um organizador de tarefas com aparência de produto real.
            </h1>
            <p className="mt-5 max-w-xl text-base leading-8 text-zinc-400">
              Fluxo limpo, visão operacional e uma estrutura visual mais madura para estudo, portfólio ou uso contínuo.
            </p>
          </div>

          <div className="space-y-4">
            {[
              "Dashboard mais clara e menos poluída",
              "Criação e edição em painel lateral",
              "Filtros, ordenação e ações refinadas",
            ].map((item) => (
              <div
                key={item}
        ...
        <section className="flex items-center justify-center px-6 py-10">
          <div className="w-full max-w-md">
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-400">
              Acesse seu workspace e acompanhe suas tarefas com uma interface mais organizada.
            </p>
